...
import { compareNormalized, toSafeUrl } from "@/lib/utils-text"

export default function TagPage({ params }: { params: { slug: string } }) {
  const tagSlug = params.slug

  // Tenta encontrar posts com a tag normalizada
  let posts = getPostsByTag(tagSlug)

  // Se não encontrar posts, tenta algumas alternativas
  if (posts.length === 0) {
    // Tenta decodificar o slug (para casos de URL-encoded)
    try {
      const decodedSlug = decodeURIComponent(tagSlug)
      if (decodedSlug !== tagSlug) {
        posts = getPostsByTag(decodedSlug)
      }
    } catch (e) {
      console.error("Erro ao decodificar slug:", e)
    }

    // Se ainda não encontrou, tenta converter hífens para espaços
    if (posts.length === 0 && tagSlug.includes("-")) {
      const tagWithSpaces = tagSlug.replace(/-/g, " ")
      posts = getPostsByTag(tagWithSpaces)
    }
  }
  ...
  // Determina o nome da tag para exibição
  // Usa a primeira tag encontrada nos posts que corresponde ao slug normalizado
  const tagName =
    posts[0].tags.find(
      (tag) => compareNormalized(tag, tagSlug) || compareNormalized(tag, tagSlug.replace(/-/g, " ")),
    ) ||
    tagSlug
      .split("-")
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-grow">
        ...
        <section className="bg-gradient-to-b from-white to-[#ededed] py-12 md:py-16">
          <div className="container mx-auto px-4">
            <div className="text-center max-w-3xl mx-auto">
              ...
              <h1 className="text-3xl md:text-5xl font-bold mb-4">Tag: {tagName}</h1>
              <p className="text-lg text-gray-700 mb-8">
                Artigos, dicas e novidades sobre {tagName.toLowerCase()} para clínicas e consultórios de saúde.
              </p>
        ...
        <section className="py-12 bg-[#ededed]">
          <div className="container mx-auto px-4">
            <h2 className="text-2xl font-bold mb-8 text-center">Tags relacionadas</h2>
            <div className="flex flex-wrap justify-center gap-4">
              {Array.from(new Set(posts.flatMap((post) => post.tags)))
                .filter((tag) => !compareNormalized(tag, tagName))
                .map((tag) => (
                  <Link
                    key={tag}
