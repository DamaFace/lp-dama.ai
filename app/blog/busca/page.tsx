...
import { searchPosts, type BlogPost } from "@/lib/blog-data"
        ...
        <section className="bg-gradient-to-b from-white to-[#ededed] py-12">
          <div className="container mx-auto px-4">
            <div className="text-center max-w-3xl mx-auto">
              <h1 className="text-3xl md:text-4xl font-bold mb-4">Resultados da busca</h1>
              <p className="text-lg text-gray-700 mb-8">
                {loading
                  ? "Buscando..."
                  : results.length > 0
                    ? `Encontramos ${results.length} ${results.length === 1 ? "resultado" : "resultados"} para "${query}"`
                    : `Nenhum resultado encontrado para "${query}"`}
              </p>

              <form action="/blog/busca" method="get" className="relative max-w-xl mx-auto">
          ...
          <div className="container mx-auto px-4">
            {loading ? (
              <div className="text-center py-12">
                <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-[#fa1571] border-r-transparent"></div>
                <p className="mt-4 text-gray-600">Buscando artigos...</p>
              </div>
            ) : results.length > 0 ? (
              ...
              <div className="text-center py-12">
                <div className="bg-gray-100 rounded-full p-6 inline-flex items-center justify-center mb-6">
                  <Search className="h-12 w-12 text-gray-400" />
                </div>
                <h2 className="text-2xl font-bold mb-2">Nenhum resultado encontrado</h2>
                <p className="text-gray-600 mb-6">
                  Não encontramos nenhum artigo correspondente à sua busca. Tente termos diferentes ou navegue por
                  nossas categorias.
                </p>
                <Link
                  href="/blog"
                  className="inline-block px-6 py-3 bg-[#fa1571] text-white rounded-lg hover:bg-opacity-90 transition-colors"
                >
                  Voltar para o blog
                </Link>
              </div>
